...
def generate_embeddings (messages_in):
    # generate embeddings
    embed_module = hub.Module('https://tfhub.dev/google/universal-sentence-encoder/2', trainable = True)
...
def get_encodes(x):
    ...
    features = generate_embeddings(text)
...
run_config = RunConfig(model_dir='./data/cips/save/uestion-uce-model-faster-finetune',
